...
/*   Updated: 2023/08/23 20:18:10 by ftuernal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	struct_alloc(t_data *data)
{
	...
	data->chop_status = malloc(data->think_nb * sizeof(int *));
	if (!data->chop_status)
		return (str_error("chop_status ** alloc failed\n", data));
	i = -1;
	while (++i < data->think_nb)
	{
		data->chop_status[i] = malloc(sizeof(int));
		if (!data->chop_status[i])
			return (str_error("Error chopstatus alloc\n", data));
	}
	return (0);
}
...
static void	fork_init(t_data *data)
{
	...
	while (++i < data->think_nb)
	{
		...
		*data->chop_status[i] = 0;
	}
	data->think[0].l_chop = &data->chopstick[0];
	data->think[0].l_status = data->chop_status[0];
	if (data->think_nb > 1)
	{
		data->think[0].r_chop = &data->chopstick[data->think_nb - 1];
		data->think[0].r_status = data->chop_status[data->think_nb - 1];
		i = 0;
		while (++i < data->think_nb)
		{
			data->think[i].l_chop = &data->chopstick[i];
			data->think[i].l_status = data->chop_status[i];
			data->think[i].r_chop = &data->chopstick[i - 1];
			data->think[i].r_status = data->chop_status[i - 1];
		}
	}
}
...
static int	init_conditions(t_data *data, char **av, int ac)
{
	...
	pthread_mutex_init(&data->release, NULL);
